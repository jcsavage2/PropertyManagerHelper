        ...
        <div className=" mb-44 md:mb-10">
          <div className={isMobile ? `w-full flex flex-col justify-center` : `flex flex-row justify-between`}>
            ...
            <div className={`justify-self-end ${isMobile && 'mt-2 w-full'}`}>
              {user?.isAdmin ? <CreatePropertyManagerModal onSuccessfulAdd={() => fetchPMs(true)} /> : null}
            </div>
          </div>
          {isMobile ? (
            <div className={`mt-1 pb-4 min-h-screen`}>
                ...
                {pms.map((pm: IUser, index) => {
                  return (
                    <div
                      ...
                      className={`flex flex-row justify-between items-center w-full rounded-lg py-4 px-2 h-36 bg-gray-100 shadow-lg ${index === 0 && 'mt-1'} ${
