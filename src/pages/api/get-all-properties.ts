...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse>) {
  try {
    ...
    if (body.pmEmail && body.organization) {
      throw new ApiError(API_STATUS.BAD_REQUEST, 'Attempting to get properties with org AND pm info');
    }
    ...
    if (body.pmEmail) {
      ...
      response = await propertyEntity.getByOrganization({ organization: body.organization!, startKey: body.startKey });
    }

    return res.status(API_STATUS.SUCCESS).json({ response: JSON.stringify({ properties: response.properties, startKey: response.startKey }) });
  } catch (error: any) {
    console.log({ error });
