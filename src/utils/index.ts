 ...
 */
export function generateKey(firstIdentifier: string, secondIdentifier: string) {
  return [firstIdentifier, secondIdentifier].join('#');
}
...
export function constructEventKey(woId: string) {
  return ENTITY_KEY.EVENT + [woId, woId].join(':');
}

export function deconstructEventKey(key: string) {
  key.split(':');
}