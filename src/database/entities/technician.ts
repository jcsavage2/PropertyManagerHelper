...
export class TechnicianEntity {
   ...
   */
  public async create({ name, email, pmEmail, organization }: CreateTechnicianProps) {
    try {
      /**
  ...
  public async delete({ pk, sk }: { pk: string; sk: string }) {
    ...
    const result = await this.technicianEntity.delete(params);
    return result;
  }

  public async update({ technicianEmail, technicianName, organization, status }: Partial<ITechnicianBase>) {
    if (!technicianEmail) {
